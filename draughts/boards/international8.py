"""
International8 - 8x8 board, flying kings, men move/capture forward only, mandatory captures.

This is a custom variant combining:
- 8x8 board (32 playable squares, like American checkers)
- Flying kings (move any number of squares diagonally, like International/Russian)
- Men move AND capture forward only (like American checkers)
- Captures are mandatory (like International/Russian)
- Maximum capture NOT required (capture any valid sequence)
"""
...
def _build_tables():
    ...
    # Build full diagonal rays for flying kings
    rays = []
    for sq in range(32):
        sq_rays = [[], [], [], []]
        for d in range(4):
...
class Board(BaseBoard):
    """
    International8 - custom 8x8 variant.

    - 8x8 board, 32 squares
    - Flying kings (move any distance diagonally)
    - Men move and capture forward only
    - Captures mandatory (but maximum capture NOT required)
    """

    GAME_TYPE = 23  # closest to American for PDN compatibility
    VARIANT_NAME = "International8 checkers"
    ...
    def legal_moves(self) -> list[Move]:
        """Captures are mandatory; if any capture exists return only captures."""
    ...
    def _gen_simple(self) -> list[Move]:
        ...
        if self.turn == Color.WHITE:
            # Men: forward only (directions 0 and 1 = towards row 0)
            even = wm & EVEN_ROWS & ~ROW[0]
            odd = wm & ODD_ROWS & ~ROW[0]
            for bb, shift in [((even & ~EVEN_RIGHT) >> 3, 3), (odd >> 4, 4),
                               (even >> 4, 4), ((odd & ~ODD_LEFT) >> 5, 5)]:
                bb &= empty
                while bb:
                    lsb = bb & -bb; t = lsb.bit_length() - 1; bb ^= lsb
                    moves.append(Move([t + shift, t]))
            # Kings: flying (all 4 directions)
            bb = wk
            while bb:
            ...
            # Men: forward only (directions 2 and 3 = towards row 7)
            even = bm & EVEN_ROWS & ~ROW[7]
            odd = bm & ODD_ROWS & ~ROW[7]
            for bb, shift in [((even & ~EVEN_RIGHT) << 5, -5), (odd << 4, -4),
                               (even << 4, -4), ((odd & ~ODD_LEFT) << 3, -3)]:
                bb = bb & empty & MASK_32
                while bb:
                    lsb = bb & -bb; t = lsb.bit_length() - 1; bb ^= lsb
                    moves.append(Move([t + shift, t]))
    ...
    def _man_captures(self, sq: int, enemy: int, captured: set[int], out: list[Move], is_white: bool) -> None:
        ...
        # White forward = directions 0,1 (towards row 0); Black forward = directions 2,3 (towards row 7)
        dirs = (0, 1) if is_white else (2, 3)
        for d in dirs:
    ...
    def _king_captures(self, sq: int, enemy: int, captured: set[int], forbidden: set[int], out: list[Move]) -> None:
        """Flying kings capture in all 4 directions."""
        wm, wk, bm, bk = self.white_men, self.white_kings, self.black_men, self.black_kings
        all_p, src_bit = wm | wk | bm | bk, 1 << sq
